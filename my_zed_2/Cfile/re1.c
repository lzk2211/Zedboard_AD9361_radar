...
/****************************************/
    ...
    int kk;
    for(int k=0;k<8;k++)
        tx_proxy_interface_p->buffer[k] = 0x00;
    for(int k=2;k<1026;k++)
    {
        kk = k - 2;
        // tx_proxy_interface_p->buffer[k*2] = real_data[k];
        // tx_proxy_interface_p->buffer[k*2+1] = imag_data[k];

        tx_proxy_interface_p->buffer[k*4+1]   = (real_data[kk*4]<<4) + real_data[kk*4+1];
        tx_proxy_interface_p->buffer[k*4] = (real_data[kk*4+2]<<4) + real_data[kk*4+3];

        tx_proxy_interface_p->buffer[k*4+3] = (imag_data[kk*4]<<4) + imag_data[kk*4+1];
        tx_proxy_interface_p->buffer[k*4+2] = (imag_data[kk*4+2]<<4) + imag_data[kk*4+3];
