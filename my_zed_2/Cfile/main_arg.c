...
int main(int argc, char **argv)
{
    ...
    // if (argc != 2) {
	// 	printf("Usage: data-to-win <# of interval sampling number n (n*1024)>\n");
	// 	// exit(EXIT_FAILURE);
	// }
    // else{
    //     tx_proxy_interface_p->length = 4104;
    //     for(int k=0;k<7;k++)
    //         tx_proxy_interface_p->buffer[k] = 0xff;

    //     tx_proxy_interface_p->buffer[7] = atoi(argv[1]);
    //     for(int k=8;k<tx_proxy_interface_p->length;k++)
    //         tx_proxy_interface_p->buffer[k] = 0xff;
    //     // printf("%d\t%d", tx_proxy_interface_p->buffer[7], atoi(argv[1]));
